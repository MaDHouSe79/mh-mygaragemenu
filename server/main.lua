...
QBCore.Functions.CreateCallback('mh-mygaragemenu:server:isOwner', function(source, cb, plate)
    ...
    local result = MySQL.query.await('SELECT plate, citizenid FROM player_vehicles WHERE plate = ? AND citizenid = ?',
...
QBCore.Functions.CreateCallback("mh-mygaragemenu:server:getMyVehicles", function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.Async.fetchAll('SELECT * FROM player_vehicles WHERE citizenid = ?', {Player.PlayerData.citizenid},
        function(result)
            if result[1] ~= nil then
                cb(result)
            else
...
RegisterNetEvent('mh-mygaragemenu:server:GetVehicleOutGarage', function(plate)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.Async.execute('UPDATE player_vehicles SET state = ? WHERE plate = ? AND citizenid = ?',
...
RegisterNetEvent('mh-mygaragemenu:server:PutVehicleInGarage', function(plate)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.Async.execute('UPDATE player_vehicles SET state = ? WHERE plate = ? AND citizenid = ?',
        {1, plate, Player.PlayerData.citizenid})
end)
